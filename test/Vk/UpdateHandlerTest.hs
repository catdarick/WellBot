module Vk.UpdateHandlerTest where
...
testRepeat = TestCase (assertEqual "testRepeat failed" expectedRes res)
  where
    ...
    initDb = getDb False False 0 
    initUpdate = getUpdateWithMessage "/repeat"
    expectedDb = getDb True False 0 
    expectedAction =
      runIdentity $
      testSendKeyboard config userId_ ((config & repeatText) ++ show 1)
...
testKeyboardResponse =
  ...
  where
    res = inlinePerformIO $ runStateT (handleUpdate h config initUpdate) initDb
    initDb = getDb True False 0 
    initUpdate = getUpdateWithMessage "2"
    expectedDb = getDb False True 2 
    expectedAction = ("", "")
    expectedRes = (expectedAction, expectedDb)
...
testForwardDefault =
  ...
  where
    ...
    expectedDb = getDb False False 0 
    expectedAction =
      runIdentity $
      testForwardNTimes config userId_ messageId_ (config & defaultRepeatAmount)
    expectedRes = (expectedAction, expectedDb)
...
testHelp = TestCase (assertEqual "testForwardDefault failed" expectedRes res)
  where
    ...
    expectedDb = getDb False False 0 
    expectedAction =
      runIdentity $ testSendMessage config userId_ (config & helpText)
    expectedRes = (expectedAction, expectedDb)
...
testForward2Times =
  ...
  where
    res = inlinePerformIO $ runStateT (handleUpdate h config initUpdate) initDb
    initDb = getDb False True 2 
    initUpdate = getUpdateWithMessage "smt"
    expectedDb = getDb False True 2 
    expectedAction = runIdentity $ testForwardNTimes config userId_ messageId_ 2
    expectedRes = (expectedAction, expectedDb)
